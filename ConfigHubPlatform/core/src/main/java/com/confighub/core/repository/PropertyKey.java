...
@NamedQueries(
      {
            @NamedQuery( name = "Key.getByKey",
                         query = "SELECT k FROM PropertyKey k WHERE k.repository=:repository AND UPPER(k.key)=:key" ),

            @NamedQuery( name = "Key.getById",
                         query = "SELECT k FROM PropertyKey k WHERE k.repository=:repository AND k.id=:keyId" ),

            @NamedQuery( name = "Key.search",
                         query = "SELECT k FROM PropertyKey k WHERE k.repository=:repository AND UPPER(k.key) LIKE :searchTerm" ),

            @NamedQuery( name = "Key.getNonText",
                         query = "SELECT k FROM PropertyKey k WHERE k.repository=:repository AND k.valueDataType <> :type" ),

            @NamedQuery( name = "Key.getKeys",
                         query = "SELECT k FROM PropertyKey k WHERE k.repository=:repository AND UPPER(k.key) LIKE :keys" ),
