...
@Table( name = "repository",
        uniqueConstraints = @UniqueConstraint( columnNames = { "name",
                                                               "accountId" } ),
        indexes = { @Index( name = "REPO_repoIndex",
                            columnList = "id, name, accountId" ) }
)
@NamedQueries(
      {
            @NamedQuery( name = "Level.byId",
                         query = "SELECT l FROM CtxLevel l WHERE l.repository=:repository AND l.id=:id" ),

            @NamedQuery( name = "repository.get",
                         query = "SELECT c FROM Repository c WHERE c.id=:id" ),

            @NamedQuery( name = "Repository.getByAccount",
                         query = "SELECT p FROM Repository p WHERE p.name=:name AND p.account=:account AND p.active=true" ),

            @NamedQuery( name = "Repository.count",
                         query = "SELECT COUNT(c) FROM Repository c WHERE c.active=true" ),

            @NamedQuery( name = "Repository.getAll",
                         query = "SELECT r FROM Repository r WHERE r.active=true" )
      } )
@EntityListeners( { RepositoryDiffTracker.class } )
@Audited
@AuditTable( "repository_audit" )
      ...
      extends APersisted
{
    ...
    @OneToMany( fetch = FetchType.LAZY,
                cascade = { CascadeType.REMOVE },
                mappedBy = "repository" )
    private Set<CtxLevel> ctxLevels;
    ...
    public UserType getUserType( final UserAccount user )
    {
        ...
        {
            if ( this.account.getOrganization().isAdmin( user ) )
            {
                return UserType.admin;
            }
            if ( this.account.getOrganization().isOwner( user ) )
            {
                return UserType.owner;
            }
        }
    ...
    public AccessRuleWrapper getRulesWrapper( final UserAccount user,
                                              final AccessRule.RuleTarget filter )
    {
    ...
    public void transferOwnershipAccount( final Account newAccount,
    ...
    public void setDepthLabel( Depth depth,
                               String label )
          throws ConfigException
    {
    ...
    public Set<CtxLevel> getCtxLevels()
    {
        return ctxLevels;
    }
