...
@NamedQueries(
      {
            @NamedQuery( name = "Team.get",
                         query = "SELECT t from Team t WHERE t.repository=:repository AND t.name=:name" ),

            @NamedQuery( name = "Team.forMember",
                         query = "SELECT t from Team t WHERE t.repository=:repository AND :member MEMBER of t.members" )
      } )
@Audited
